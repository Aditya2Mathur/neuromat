...
export default function Dashboard() {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      setRecentQueue(queue.slice(0, 10))
    } catch (e) { console.error(e) }
    finally { setLoading(false) }
  }

  const STAT_CARDS = [
    { label: "Today's Patients", value: stats.todayPatients, icon: Users,        color: '#6366f1', trend: true },
    { label: 'Pending Queue',    value: stats.pendingQueue,  icon: Queue,        color: '#f59e0b' },
    ...
    { label: 'Completed Today',  value: stats.completedToday,icon: CheckCircle,  color: '#8b5cf6' },
  ]
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div className="card-header">
          <div>
            <h3 style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-primary)' }}>Today's Queue</h3>
            <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>Live patient status updates</p>
          </div>
